...
describe 'basic installation' do
  ...
  context 'with basic parameters' do
    ...
    describe command('podman run --rm --network=iop-core-network quay.io/iop/puptoo:foreman-3.18 curl http://iop-core-puptoo:8000/metrics') do
      its(:exit_status) { should eq 0 }
    end

    describe command('podman run --rm --network=iop-core-network quay.io/iop/yuptoo:foreman-3.18 curl http://iop-core-yuptoo:5005/') do
      its(:exit_status) { should eq 0 }
    end

    describe command('podman run --rm --network=iop-core-network quay.io/iop/ingress:foreman-3.18 curl http://iop-core-ingress:8080/') do
      its(:exit_status) { should eq 0 }
    end
    ...
    describe command('podman run --rm --network=iop-core-network quay.io/iop/host-inventory:foreman-3.18 curl http://iop-core-host-inventory:9126/') do
      its(:exit_status) { should eq 0 }
    end
  ...
  context 'when registering as a smartproxy' do
    ...
    describe command("podman run --rm --network=iop-core-network quay.io/iop/puptoo:foreman-3.18 curl -s -o /dev/null -w '%{http_code}' http://iop-core-puptoo:8000/metrics") do
      its(:stdout) { should match /200/ }
    end

    describe command("podman run --rm --network=iop-core-network quay.io/iop/yuptoo:foreman-3.18 curl -s -o /dev/null -w '%{http_code}' http://iop-core-yuptoo:5005/") do
      its(:stdout) { should match /200/ }
    end

    describe command("podman run --rm --network=iop-core-network quay.io/iop/ingress:foreman-3.18 curl -s -o /dev/null -w '%{http_code}' http://iop-core-ingress:8080/") do
      its(:stdout) { should match /200/ }
    end

    describe command("podman run --rm --network=iop-core-network quay.io/iop/host-inventory:foreman-3.18 curl -s -o /dev/null -w '%{http_code}' http://iop-core-host-inventory:9126/") do
      its(:stdout) { should match /200/ }
    end

    describe command("podman run --rm --network=iop-core-network quay.io/iop/host-inventory:foreman-3.18 curl -s -o /dev/null -w '%{http_code}' http://iop-core-host-inventory-api:8081/health") do
      its(:stdout) { should match /200/ }
    end
